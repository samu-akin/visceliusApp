...
import { ScrollArea } from "./components/ui/scroll-area" // Updated path
import { Avatar, AvatarFallback, AvatarImage } from "./components/ui/avatar" // Updated path
...
import {
  ...
  Pause,
  StopCircle,
  Sparkles // Novo ícone para IA
} from 'lucide-react';
import { Tabs, TabsContent, TabsList, TabsTrigger } from "./components/ui/tabs" // Updated path
import { Textarea } from "./components/ui/textarea" // Updated path
import { Calendar as CalendarUI } from "./components/ui/calendar" // Updated path
import { Popover, PopoverContent, PopoverTrigger } from "./components/ui/popover" // Updated path
...
const musicoterapiasMock: Musicoterapia[] = [
  { id: 'mt1', titulo: 'Melodias para Relaxamento Profundo', descricao: 'Sons suaves e harmônicos para induzir um estado de calma.', tipo: 'Relaxamento', duracaoMin: 30 },
  { id: 'mt2', titulo: 'Ritmos para Foco e Concentração', descricao: 'Batidas binaurais e música instrumental para melhorar a atenção.', tipo: 'Cognitiva', duracaoMin: 45 },
  { id: 'mt3', titulo: 'Harmonias para Alívio da Ansiedade', descricao: 'Peças clássicas e ambientais que promovem bem-estar.', tipo: 'Bem-estar', duracaoMin: 25 },
  { id: 'mt4', titulo: 'Sons da Natureza para Meditação', descricao: 'Gravações de florestas, rios e pássaros para práticas meditativas.', tipo: 'Meditação', duracaoMin: 60 },
  { id: 'mt5', titulo: 'Musicoterapia Energizante Matinal', descricao: 'Músicas vibrantes e ritmadas para começar o dia com energia.', tipo: 'Energia', duracaoMin: 20 },
];
...
const App = () => { // Renamed from VisceliusApp to App
  ...
  const intervalRef = useRef<number | null>(null); // Use useRef for interval ID
  ...
  const handlePlayMusicGlobal = useCallback((terapia: Musicoterapia, resumeFrom: number = 0) => {
    ...
    setPlaybackProgressGlobally((resumeFrom / (terapia.duracaoMin * 60)) * 100);
    setElapsedTimeGlobally(resumeFrom);

    const totalSeconds = terapia.duracaoMin * 60;
    const updateInterval = 100; // Update every 100ms for smooth progress
    let currentElapsed = resumeFrom;

    const newIntervalId = setInterval(() => {
      currentElapsed += (updateInterval / 1000); // Add elapsed time in seconds
      const newProgress = (currentElapsed / totalSeconds) * 100;
      ...
      if (newProgress >= 100) {
        ...
        setIsPlayingGlobally(false);
        setCurrentPlayingTrackGlobally(null);
        setPlaybackProgressGlobally(100); // Ensure it reaches 100%
        setElapsedTimeGlobally(totalSeconds); // Ensure full duration
        setGlobalMessage(`Reprodução concluída: ${terapia.titulo}`);
        setTimeout(() => setGlobalMessage(null), 3000); // Clear message after completion
      } else {
        setPlaybackProgressGlobally(newProgress);
        setElapsedTimeGlobally(currentElapsed);
      }
    }, updateInterval);

    intervalRef.current = newIntervalId as unknown as number; // Store interval ID
    setGlobalMessage(`Reproduzindo: ${terapia.titulo}`);
    setTimeout(() => setGlobalMessage(null), 3000); // Clear message after starting
  }, []);
  ...
  const handlePauseResumeMusicGlobal = useCallback(() => {
    if (isPlayingGlobally) {
      ...
      // Resume
      if (currentPlayingTrackGlobally) {
        handlePlayMusicGlobal(currentPlayingTrackGlobally, elapsedTimeGlobally);
      }
    }
  }, [isPlayingGlobally, currentPlayingTrackGlobally, elapsedTimeGlobally, handlePlayMusicGlobal]);


  // Cleanup effect for the global player interval
  useEffect(() => {
    return () => {
      if (intervalRef.current !== null) {
        clearInterval(intervalRef.current);
      ...
      <AnimatePresence>
        {currentPlayingTrackGlobally && ( // Render only if a track is set (playing or paused)
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 50 }}
            className="fixed bottom-0 left-0 right-0 bg-gray-800/90 backdrop-blur-sm p-4 border-t border-white/10 flex items-center justify-between z-40"
          >
